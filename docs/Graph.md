...
* Complete: one node per ingested event — appended after classification even when Jev is degraded (level `max(prior, jev, gate.incident_level)`, confidence 0). The flagged subset is `key_nodes()` (level ≥ 1).
